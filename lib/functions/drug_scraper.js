...
const scrapePageData = async (page) => {
    ...
    return page.evaluate(() => {
        ...
        const data = {
            genericName: getTextContent('h1'),
            genericAndBrandNames: [getTextContent('h1').split(' ')[0]],
            drugType: null,
            drugPurpose: null,
            drugAdvice: null,
            ...
            rows.forEach(row => {
                ...
                if (rowHeader && rowContent) {
                    if (rowHeader.includes('Type of medicine')) {
                        ...
                        data.genericAndBrandNames.push(...rowContent
                            .split(';')
                            .map(name => name.replace(/®/g, '').trim()).filter(name => name !== ''));
                    }
                }
            });
        }
...
const saveToFirestore = async (drugData) => {
    const docRef = db.collection('drugs').doc(drugData.genericName);
    await docRef.set({
        genericName: drugData.genericName,
        genericAndBrandNames: drugData.genericAndBrandNames,
        drugType: drugData.drugType,
        drugPurpose: drugData.drugPurpose,
        drugAdvice: drugData.drugAdvice,
